...
After carry lands on a freshly cut worktree, `--cut-worktree` runs a repo-root `.anvil-worktree-hook` when present — a tracked, executable escape hatch for state carry can't express, such as deriving a worktree-local `.env` from a read-only agent principal rather than copying the repo's own. It runs synchronously with cwd at the repo root and the absolute worktree path as `$1`, so it must finish (and spawn nothing that outlives it) before the cut returns — a lingering process wedges a later `git worktree remove`. The hook runs as its own process group and an interrupt to anvil itself (Ctrl-C) is forwarded into that group too, so a refusal or an interrupt kills every descendant, not just the direct child.

A missing hook is a silent no-op, no refusal. Otherwise there are four refusal codes, all running the same worktree/branch cleanup:

- Present but not executable: `cut_worktree_hook_not_executable`.
- Non-zero exit: `cut_worktree_hook_failed` (carries the hook's stderr tail); removes the worktree and branch the cut just made, so a failing hook orphans nothing.
- Still running after 30s: `cut_worktree_hook_timeout`, killing the whole process group so the refusal isn't wedged by an orphaned descendant.
- Exits 0 but leaves a descendant holding stderr open past a 5s WaitDelay: `cut_worktree_hook_orphan_process`, after killing that descendant — the hook succeeded, but the daemon it left behind is exactly what "spawn nothing that outlives it" forbids.

A reused worktree skips the hook, same as carry. Anything the hook writes into the worktree must be gitignored, for the same reason carry's declared paths must be: a non-ignored write makes `--land-pr`'s post-merge `git worktree remove` refuse, even on a hook that succeeds.
