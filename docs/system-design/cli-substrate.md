...
**Skills auto-refresh.** `anvil install skills` materialises the binary's embedded skill bundle to `<config-dir>/.anvil-skills-src/` (the resolved `--target` config dir — `~/.claude` or `~/.codex`, honoring `CLAUDE_CONFIG_DIR`/`CODEX_HOME`; `ANVIL_SKILLS_DIR` overrides outright) and writes a content hash to `.anvil-skills-hash` alongside it. Install, uninstall, `anvil init --install-claude`, and the auto-refresh all resolve that dir through the same helper, so exactly one materialise dir exists per target. Every non-`install` invocation compares the Claude target's on-disk hash against the embedded FS; on mismatch (typically after `go install ./cmd/anvil` rebuilt the binary with edited SKILL.md content), it rewrites the bundle in place and prints a one-line stderr notice. The check no-ops until a first install creates the dir; a stale Codex bundle refreshes only via an explicit `anvil install skills --target codex`.
