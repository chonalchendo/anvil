...
| Lint | `golangci-lint v2` with `linters.default: standard` + `errorlint`, `gocritic`, `revive`, `misspell`, `gosec`, `bodyclose`, `nilerr`, `contextcheck` | Configured in `.golangci.yml`; runs in CI and via `prek` (see below). Use `golangci-lint fmt` instead of separate gofumpt. |
