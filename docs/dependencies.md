...
| Lint | `golangci-lint v2` with `linters.default: standard` (includes `errcheck`) + `errorlint`, `gocritic`, `revive`, `misspell`, `bodyclose`, `nilerr`, `contextcheck`, `gosec` | Configured in `.golangci.yml`; runs in CI and via `prek` (see below). Use `golangci-lint fmt` instead of separate gofumpt. Cobra/`fmt` writer noise is excluded once via `linters.settings.errcheck.exclude-functions`, not per-call-site `//nolint`. |
