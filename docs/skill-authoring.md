...
- **On-demand body: `**REQUIRED REFERENCE:** Use skills/<name>/references/<mode>.md`.** Points at a `references/` file in the same skill directory. The agent loads it when that mode/kind is needed; the always-on SKILL.md stays lean. Use when a skill branches by kind (e.g. bug vs feature) or mode (e.g. fast vs deep) and each branch carries enough content to push the body over the 200-line target. Differs from `REQUIRED SUB-SKILL` in that it loads a file, not a skill — no trigger contract, no frontmatter, no registration; just a markdown body the agent reads inline. Use `REQUIRED REFERENCE` for content that belongs to *this* skill's body but doesn't need its own trigger surface; use `REQUIRED SUB-SKILL` when you're handing control to a separate, independently-triggerable skill.
