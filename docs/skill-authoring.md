...
- **On-demand body: `**REQUIRED REFERENCE:** Use skills/<name>/references/<mode>.md`.** Points at a `references/` file in the same skill directory. The agent loads it when that mode/kind is needed; the always-on SKILL.md stays lean. Use when a skill branches by kind (e.g. bug vs feature) or mode (e.g. fast vs deep), or when a phase's rationale is read once per run but costs context on every run, and the content carries the body over the 200-line target. Differs from `REQUIRED SUB-SKILL` in that it loads a file, not a skill — no trigger contract, no frontmatter, no registration; just a markdown body the agent reads inline.
