...
- **Any test executing a root command with `--vault`/`--project` must pin the selector.** The root command's `PersistentPreRunE` exports those flags into the process environment, so an unpinned test leaks its vault into whichever test runs next — a failure that only reproduces under `-shuffle`. Call `isolateRootEnv(t)` (or route through `setupVault`/`runCmd`/`runArgs`/`execCmd`/`createIssueGetPath`, which all call it) so `t.Setenv` restores the value at cleanup. The `CLI test isolation` step in `.github/workflows/ci.yml` greps the test tree and fails the build on a func that misses this.
