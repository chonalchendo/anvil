...
Choose the `--body` by whether the target repo defines a PR template — `--body` is always passed (GitHub only auto-populates `.github/PULL_REQUEST_TEMPLATE.md` when it is *absent*, and the issue citation must survive either way):
...
# emits one JSON on terminal state: {state, merged, ci_conclusion, review_blockers_count, timed_out}; state: merged|closed|conflicting|ci_passed|review_blocked|ci_failed|timeout — branch handling is in responding-to-pr-review Phase 4
```
...
## Phase 6 — Distil the run (on resolve)

A completed run is the system's most learning-rich event — a verified change just landed. Once the issue reaches `resolved` (the human fired the merge+resolve verb in Phase 5), **auto-distil compounding learnings** via `distilling-learning`'s attended autonomous mode — do not offer or ask. Hold its **high-value bar**: distil only when you can name the specific future failure the learning prevents; most runs clear it for *nothing*, and a marginal learning is negative value — it pollutes retrieval and hands the next agent worse context — so default to silence.

**Dispatched / autonomous mode is a no-op.** A fleet- or `anvil-issue-worker`-dispatched completion stops at PR-opened and never reaches the resolve, so it neither distils nor offers — consistent with the dispatched-completion-stops-at-PR-opened convention. No human is present to see what would land, so the orchestrator owns any post-merge distillation (staged, then human-harvested).
