...
**Autonomous mode (unattended runs):** when the caller declares an unattended run, do not halt for the human — file a blocker for the scope-change (an `inbox` item, or a `blocks` issue when a milestone fits), leave the issue `in-progress`, note it in the report, and move on to the next. Verification non-convergence already reports rather than hangs (Phase 5). Every other gate — `## Verification`, the build-and-install gate, the independent review — still runs; never auto-merge.
