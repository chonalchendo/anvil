...
## Phase 6 — Post-merge distillation harvest

Phase 2b is the read side; this is the write side. Workers stop at PR-opened, never reaching `completing-issue`'s distillation step — so learnings they surface are lost unless harvested here, once the batch lands.
...
2. **Auto-distil the compounding candidates** via `distilling-learning`'s attended autonomous mode — no offer. Hold its **high-value bar**: distil a candidate only when you can name the specific future failure it prevents; drop the rest silently. Most batches yield one or none — a marginal learning pollutes retrieval, so default to silence over completeness.

**Autonomous orchestrator:** does **not** auto-distil — no human is present to see what would land, so stage candidates into the report under `Harvest candidates:` and stop. A human fires `distilling-learning` later.
