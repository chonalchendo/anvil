...
- **No early-exit pipe over a streaming side-effect command** — `producer | grep -q X` (or `| head`) closes the pipe on first match and SIGPIPE-kills `producer` before its side effect completes, so the predicate can pass while the side effect never happened. Capture the producer's output first, then assert on the capture: `producer > out; grep -q X out`.
