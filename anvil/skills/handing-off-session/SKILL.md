...
`anvil session handoff` resolves *this terminal's* session from `$CLAUDE_CODE_SESSION_ID` (no mtime guessing), preserves the frontmatter, and writes your body verbatim. Under Codex (no `$CLAUDE_CODE_SESSION_ID`) it binds to the active Codex session via the newest `$CODEX_HOME/sessions/**/rollout-*.jsonl` and creates the session file on first handoff, since Codex has no SessionStart hook to pre-create it. Because the path is derived from your own session id, each terminal writes only to its own file — that is what prevents one session from clobbering a concurrent session's unconsumed handoff. As an integrity backstop it also **refuses** the write if the file at that path stores a *different* `session_id` (a renamed or hand-edited file). If it refuses, stop and surface the message — do not hand-edit the file or write it directly.
