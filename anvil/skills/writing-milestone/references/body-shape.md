...
- `## Objective`
  ...
  - **Waves** — bold run-in label, then a numbered list. A wave is one line (bold name, then its intent in a few words) followed by one nested bullet per issue: the `[[wikilink]]`, then what lands in ≤15 words. Add ids as issues are written. Wave rationale is one sentence on the wave line or moves into the issue. Wave order becomes typed edges on the issues (`writing-issue` Phase 4b), never prose alone.
...
One idea per sentence, about 20 words, no chained clauses. A paragraph stays under about 80 words; one that would enumerate is a list. CommonMark only: no callouts or other Obsidian-only syntax.

**Cold-reader test.** Cover everything after the Objective's first sentence. Can a reader say what ships?

**Glance test.** Read only the labels, wave names, nested issue links and the Status table. Can the reader say what ships, what has landed, and what to work next? A wave line holding more than one issue link fails it.
