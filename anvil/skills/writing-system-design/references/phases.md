# Writing System Design — Phases

Each phase has an explicit user gate. Don't skip the gates. Phases 1, 4, and 7 are load-bearing — Phase 1 enforces the product-design dependency, Phase 4 derives components from product-design milestones, Phase 7 produces the invariants that downstream planning and review check against. The SKILL.md quick-reference table is the index; this file owns the procedure.
...
All phases walked and the artifact validated. Return to `SKILL.md` — there is no further procedure; the skill is complete.
