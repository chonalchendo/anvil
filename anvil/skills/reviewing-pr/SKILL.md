...
The underlying bar is: code a human or agent can reason about — atomic, composable, simple. Atomic means one concern in one place; composable means parts snap together without hidden coupling; simple means the least machinery that solves the problem. Instruct the subagent to measure the diff against this bar, not just against documented rules.
