...
This section encodes harness behaviour, not skill behaviour: it is duplicated in the `anvil-issue-worker` and `anvil-researcher` agent contracts and the `dispatching-issue-fleet` skill's `subagent-prompt.md` — edit all four together.
