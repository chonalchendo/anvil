...
This section encodes harness behaviour, not skill behaviour: it is duplicated in the PR-reviewer agent contract and the fleet-dispatch subagent prompt — edit all three together.
...
This section encodes harness behaviour, not skill behaviour: it is duplicated in the other two dispatched-worker contracts (the PR-reviewer agent, the fleet-dispatch subagent prompt) — edit all three together.
